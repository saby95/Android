...
public class AIHard {
    public int hardAI(int[] gamestate,HashMap<Integer,ArrayList<int[]>> winningmap){
        ...
        for (int a=0;a<7;a++){
            weights[a]+=(twoconf[a]*3);
        }

        int oppconf[] = twoCoinMatch(0,gamestate,winningmap);
        for (int a=0;a<7;a++){
            weights[a]+=(oppconf[a]*2);
        }
